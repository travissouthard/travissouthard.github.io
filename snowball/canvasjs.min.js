...
this.height=P.height),H="center");J.x=w;J.y=v;J.angle=D;J.horizontalAlign=H;this._textBlock=J;h.layoutManager.registerSpace(K,{width:this.width+("left"===K||"right"===K?this.margin+2:0),height:this.height+("top"===K||"bottom"===K?this.margin+2:0)});this.bounds={x1:w,y1:v,x2:w+this.width,y2:v+this.height};this.ctx.textBaseline="top"}};ya.prototype.render=function(){this._textBlock&&this._textBlock.render(!0)};pa(Ia,U);Ia.prototype.setLayout=ya.prototype.setLayout;Ia.prototype.render=ya.prototype.render;
Va.prototype.get=function(h,m){var s=null;0<this.pool.length?(s=this.pool.pop(),Ma(s,h,m)):s=ua(h,m);return s};Va.prototype.release=function(h){this.pool.push(h)};pa(Ja,U);var Pa={addTheme:function(h,s){cb[h]=s},addColorSet:function(h,s){za[h]=s},addCultureInfo:function(h,s){Ka[h]=s},formatNumber:function(h,s,w){w=w||"en";if(Ka[w])return da(h,s||"#,##0.##",new Ja(w));throw"Unknown Culture Name";},formatDate:function(h,s,w){w=w||"en";if(Ka[w])return Aa(h,s||"DD MMM YYYY",new Ja(w));throw"Unknown Culture Name";
...
this.overlaidCanvasCtx.textBaseline="top",Ca(this.overlaidCanvasCtx)),this._eventManager=new Y(this),this.windowResizeHandler=R(window,"resize",function(){b._updateSize()&&b.render()},this.allDOMEventHandlers),this._toolBar=document.createElement("div"),this._toolBar.setAttribute("class","canvasjs-chart-toolbar"),H(this._toolBar,{position:"absolute",right:"1px",top:"1px"}),this._canvasJSContainer.appendChild(this._toolBar),this.bounds={x1:0,y1:0,x2:this.width,y2:this.height},R(this.overlaidCanvas,
...
a[l].x-a[t].x))*(d-1)/2+1;var C=(a[l].x-a[f].x)/b;b=(a[l].y-a[f].y)/b;c[c.length]=a[t].x>a[f].x&&0<C||a[t].x<a[f].x&&0>C?{x:a[t].x+C/3,y:a[t].y+b/3}:{x:a[t].x,y:a[t].y+b/9};t=e;f=0===t?0:t-1;l=t===a.length-1?t:t+1;b=Math.abs((a[l].x-a[f].x)/(0===a[t].x-a[f].x?0.01:a[t].x-a[f].x))*(d-1)/2+1;C=(a[l].x-a[f].x)/b;b=(a[l].y-a[f].y)/b;c[c.length]=a[t].x>a[f].x&&0<C||a[t].x<a[f].x&&0>C?{x:a[t].x-C/3,y:a[t].y-b/3}:{x:a[t].x,y:a[t].y-b/9};c[c.length]=a[e]}return c}function D(a,d,c,b,e,f,l,t,C,k){var n=0;k?
(l.color=f,t.color=f):k=1;n=C?Math.abs(e-c):Math.abs(b-d);n=0<l.trimLength?Math.abs(n*l.trimLength/100):Math.abs(n-l.length);C?(c+=n/2,e-=n/2):(d+=n/2,b-=n/2);var n=1===Math.round(l.thickness)%2?0.5:0,p=1===Math.round(t.thickness)%2?0.5:0;a.save();a.globalAlpha=k;a.strokeStyle=t.color||f;a.lineWidth=t.thickness||2;a.setLineDash&&a.setLineDash(J(t.dashType,t.thickness));a.beginPath();C&&0<t.thickness?(a.moveTo(b-l.thickness/2,Math.round((c+e)/2)-p),a.lineTo(d+l.thickness/2,Math.round((c+e)/2)-p)):
0<t.thickness&&(a.moveTo(Math.round((d+b)/2)-p,c+l.thickness/2),a.lineTo(Math.round((d+b)/2)-p,e-l.thickness/2));a.stroke();a.strokeStyle=l.color||f;a.lineWidth=l.thickness||2;a.setLineDash&&a.setLineDash(J(l.dashType,l.thickness));a.beginPath();C&&0<l.thickness?(a.moveTo(b-n,c),a.lineTo(b-n,e),a.moveTo(d+n,c),a.lineTo(d+n,e)):0<l.thickness&&(a.moveTo(d,c+n),a.lineTo(b,c+n),a.moveTo(d,e-n),a.lineTo(b,e-n));a.stroke();a.restore()}function K(a,d){K.base.constructor.call(this,"Legend","legend",d,null,
a);this.chart=a;this.canvas=a.canvas;this.ctx=this.chart.ctx;this.ghostCtx=this.chart._eventManager.ghostCtx;this.items=[];this.optionsName="legend";this.height=this.width=0;this.orientation=null;this.dataSeries=[];this.bounds={x1:null,y1:null,x2:null,y2:null};"undefined"===typeof this.options.fontSize&&(this.fontSize=this.chart.getAutoFontSize(this.fontSize));this.lineHeight=Ya(this.fontFamily,this.fontSize,this.fontWeight);this.horizontalSpacing=this.fontSize}function V(a,d,c,b){V.base.constructor.call(this,
...
D.version="v3.7.3 GA";window.CanvasJS&&(D&&!window.CanvasJS.Chart)&&(window.CanvasJS.Chart=D)})();
